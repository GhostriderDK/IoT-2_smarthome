...
########################################
# CONFIGURATION
dashboard_mac_address = b'\xB0\xA7\x32\xDE\x20\xCC' # MAC address of dashboard (Educaboard). Byte string format!
...
def get_battery_percentage():          # The battery voltage percentage
      ...
      return(pct)
                                       ...
                                       # Replace with own math. Use function in adc_sub.py
...
while True:
    ...
    if bat_pct != prev_bat_pct or sensor_value != prev_sensor_value:
        data_string = str(time.ticks_ms()) + '|' + str(bat_pct) + '|' + sensor_id + '|' + str(sensor_value) # The data to send. CHANGE IT!
